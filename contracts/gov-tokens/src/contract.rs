...
use crate::state::{
    MinterData, TokenInfo, BALANCES, LOGO, MARKETING_INFO, STAKING_ADDR, TOKEN_INFO,
};
...
pub fn execute_burn(
    ...
    // TODO: Remove key from balances map since it is no longer going to be able to recieve votes
    // as user destroys the wallet
    BALANCES.update(
        deps.storage,
        &info.sender,
        |balance: Option<Uint128>| -> StdResult<_> {
            Ok(balance.unwrap_or_default().checked_sub(to_burn)?)
        },
    )?;
    // reduce total_supply
...
fn ensure_is_staking_contract(deps: Deps, contract: &str) -> Result<Addr, ContractError> {
    let staking_addr = STAKING_ADDR.load(deps.storage)?;
...
pub fn execute_send(
    ...
    let staking_addr = ensure_is_staking_contract(deps.as_ref(), &contract)?;
...
mod tests {
    ...
    fn burn_success() {
        ...
        let remainder = amount1.checked_sub(amount1).unwrap();
        assert_eq!(
            query_token_info(deps.as_ref()).unwrap().total_supply,
            remainder
        );
    ...
    fn send() {
        ...
        let info = mock_info(addr1.as_ref(), &[]);
        let env = mock_env();
        // TODO must be accounts already have balance
        let msg = ExecuteMsg::Send {
            contract: contract.clone(),
            amount: too_much,
            msg: send_msg.clone(),
